...
const Certifications = () => {
  ...
  const openModal = (image) => { if (image) { setSelectedImage(image); setModalIsOpen(true); } };
  const closeModal = () => { setModalIsOpen(false); setSelectedImage(null); };

  const nextCert = () => setCurrentIndex((p) => Math.min(p + perPage, maxIndex));
  ...
  const handleKey = useCallback((e) => {
    if (modalIsOpen) { if (e.key === 'Escape') closeModal(); }
    else { if (e.key === 'ArrowRight') nextCert(); if (e.key === 'ArrowLeft') prevCert(); }
  ...
  const onTouchStart = (e) => { if (!ENABLE_SWIPE) return; touchStartX.current = e.touches[0].clientX; };
  const onTouchEnd = (e) => {
                  ...
                  <Parallax key={`${cert.title}-${idx}`} speed={1.03 + idx * 0.02} lag={0.08 + idx * 0.03} as="div">
                    ...
                    >
                      ...
                      <div className="flex-1 flex flex-col items-center text-center px-4 mt-4 space-y-3 overflow-hidden">
                        <h3 className="text-lg font-semibold text-white leading-tight line-clamp-3">{cert.title}</h3>
...
>
  <X size={20} strokeWidth={2.2} />
          ...
          <div className="relative z-10 h-full w-full p-4 md:p-6 flex items-center justify-center">
            {selectedImage ? (
              <img src={selectedImage} alt="Certification" loading="lazy" className="max-w-full max-h-full object-contain rounded-lg" />
            ) : (
