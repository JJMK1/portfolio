  ...
  const skillCategories = [
    {
      ...
      icon: Code,
      color: 'primary',
      skills: ['Java', 'JavaScript', 'HTML5', 'CSS', 'PHP', 'Python', 'VB.NET', 'C++', 'C#', 'React']
    },
    {
      name: 'IT Support & Tools',
      icon: Settings,
      color: 'purple',
      skills: ['PC Troubleshooting', 'Microsoft Office', 'System Administration', 'Network Configuration', 'Hardware Maintenance']
    },
    {
      name: 'Data & Database',
      icon: Database,
      color: 'primary',
      skills: ['Data Collection', 'Data Visualization', 'Data Analysis', 'Database Administration', 'MySQL', 'SQL']
    },
    {
      name: 'OS & Virtualization',
      icon: Monitor,
      color: 'purple',
      skills: ['Linux', 'Shell Scripting', 'Virtual Machines', 'Containerization', 'System Monitoring']
    },
    {
      name: 'Cybersecurity',
      icon: Shield,
      color: 'primary',
      skills: ['OSINT Investigations', 'Security Analysis', 'Threat Assessment', 'Digital Forensics', 'Security Tools']
    }
  ]
  ...
  return (
    <section id="skills" className="section-padding bg-gradient-section">
      <div className="container-custom">
        {/* Title (slightly faster = foreground emphasis) */}
        <Parallax speed={1.06}>
          <motion.div
        ...
        {/* Category cards (neutral container) */}
        <Parallax speed={1.0}>
          <div className="space-y-12">
            {skillCategories.map((category, categoryIndex) => (
              // subtle variance per card + tiny lag
              <Parallax
                key={category.name}
                speed={1.02 + (categoryIndex % 3) * 0.02}
                lag={0.06 + (categoryIndex % 3) * 0.04}
                as="div"
              >
                <motion.div
                  initial={{ opacity: 0, y: 30 }}
